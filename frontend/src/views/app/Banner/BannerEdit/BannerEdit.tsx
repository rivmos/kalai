...
injectReducer('bannerEditSlice', reducer)
...
const BannerImagesEdit = () => {
    ...
    const productData = useAppSelector(
        (state) => state.bannerEditSlice.data.categoryData
    )
    const loading = useAppSelector(
        (state) => state.bannerEditSlice.data.loading
    )

    const fetchData = (data: { id: string }) => {
        dispatch(getSingleBannerImage(data))
    }
    ...
    const popNotification = (keyword: string) => {
        toast.push(
            <Notification
                ...
                duration={2500}
            >
                Banner image successfuly {keyword}
            </Notification>,
            {
        ...
        <>
            ...
            {!loading && isEmpty(productData) && (
                <div className="h-full flex flex-col items-center justify-center">
                    ...
                    />
                    <h3 className="mt-8">No banner image found!</h3>
                </div>
            )}
        </>
    )
