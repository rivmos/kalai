...
type TaskOverviewProps = {
    data?: Record<string, ProjectOverviewChart>
    className?: string
}
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <ChartLegend
                                showBadge={false}
                                label="Total Tasks"
                                value={data[timeRange[0]].total}
                            />
                        </div>
                        <div className="flex gap-x-6">
                            <ChartLegend
                                badgeClass="bg-indigo-600"
                                label={data[timeRange[0]].series[0].name}
                                value={data[timeRange[0]].onGoing}
                            />
                            <ChartLegend
                                badgeClass="bg-emerald-500"
                                label={data[timeRange[0]].series[1].name}
                                value={data[timeRange[0]].finished}
                            />
                        </div>
                    </div>
                    <div>
