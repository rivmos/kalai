...
import { HiOutlineEye, HiOutlineTrash, HiOutlinePlay } from 'react-icons/hi'
...
const OrderColumn = ({ row }: { row: Order }) => {
    ...
    const onView = useCallback(() => {
        ...
        navigate(`/app/pout/payments-out-invoice-overview`)
    }, [navigate, row])
...
const ActionColumn = ({ row }: { row: Order }) => {
    ...
    return (
        <div className="flex justify-end text-lg">
            <Tooltip title="Pay">
                <span
                    className="cursor-pointer p-2 hover:text-green-800 text-green-500"
                    onClick={() => {}}
                >
                    <HiOutlinePlay />
                </span>
